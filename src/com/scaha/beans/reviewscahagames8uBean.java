...
@ManagedBean
@ViewScoped
public class reviewscahagames8uBean implements Serializable, MailableObject{
	...
	public String getTextBody() {
		...
		return Utils.mergeTokens(reviewscahagames8uBean.mail_nonreportedgames_body,myTokens);
	}
	...
	public void loadScahaGamesNoStatReporting(){
		...
		try{
			//first get team name
			CallableStatement cs = db.prepareCall("CALL scaha.getSCAHAGamesNoReporting8u()");
			rs = cs.executeQuery();
	...
	public void EmailManagers(){
		...
		try{
			...
			CallableStatement cs = db.prepareCall("CALL scaha.getDistinctTEamswithNonSCAHAReporting8u()");
			rs = cs.executeQuery();

			CallableStatement cs2 = db.prepareCall("CALL scaha.getSCAHAGamesNoReportingByTeamId8u(?)");
			...
			if (rs != null) {
				...
				while (rs.next()) {
					...
					if (rs2 != null) {
						...
						this.setSubject(this.teamname + " - Missing SCAHA Roster Sheet Upload");
